...
/// Resolve a peer hostname/address to a libp2p Multiaddr.
/// Returns None if DNS fails or the address resolves to our own listen port
/// (self-dial prevention).
async fn resolve_peer_to_multiaddr(addr_str: &str, local_libp2p_port: u16) -> Option<libp2p::Multiaddr> {
    // Try parsing as SocketAddr first (e.g. "1.2.3.4:11625")
    let sock_addr = if let Ok(addr) = addr_str.parse::<SocketAddr>() {
        Some(addr)
    } else {
        // Bare hostname: resolve via DNS with a default port
        // Use port 0 as placeholder — we only need the IP
        let host_with_port: String = format!("{}:0", addr_str);
        let resolved = tokio::net::lookup_host(host_with_port).await;
        match resolved {
            Ok(addrs) => {
                let first = addrs.into_iter().next();
                if let Some(ref addr) = first {
                    debug!("Resolved peer {} -> {}", addr_str, addr.ip());
                }
                // Use resolved IP but keep the original semantics
                first
            }
            Err(e) => {
    ...
    // Compute the QUIC port the remote peer listens on:
    // If addr has a real port (from SocketAddr parse), use it + 1000.
    // If from DNS (port=0), use local_libp2p_port as convention.
    let remote_quic_port = if addr.port() > 0 {
        addr.port() + 1000
    } else {
        local_libp2p_port
    };

    // Self-dial prevention: skip if the resolved IP is a local address
    // and the port matches our own listen port
    if remote_quic_port == local_libp2p_port && is_local_address(addr.ip()) {
        debug!(
            "Skipping self-dial to {} (resolved to local {}:{})",
            addr_str,
            addr.ip(),
            remote_quic_port
        );
...
/// Check if an IP address is local (loopback or matches any local interface).
fn is_local_address(ip: std::net::IpAddr) -> bool {
    match ip {
        std::net::IpAddr::V4(v4) => {
            ...
            // Check if this IP belongs to a local interface by attempting
            // to bind a UDP socket to it. If bind succeeds, it's ours.
            std::net::UdpSocket::bind(std::net::SocketAddrV4::new(v4, 0)).is_ok()
        }
        std::net::IpAddr::V6(v6) => {
            if v6.is_loopback() || v6.is_unspecified() {
                return true;
            }
            std::net::UdpSocket::bind(std::net::SocketAddrV6::new(v6, 0, 0, 0)).is_ok()
        }
    }
}
...
struct App {
    ...
    /// Configured peers for periodic reconnection (hostnames/IPs from Core)
    configured_peers: Arc<RwLock<Vec<String>>>,
    /// The QUIC listen port (peer_port + 1000) used to detect self-dial
    libp2p_port: u16,
}
...
impl App {
    async fn new(config: Config, listen_mode: bool) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self {
            ...
            configured_peers: Arc::new(RwLock::new(Vec::new())),
        })
    }
    ...
    async fn run(mut self) {
        ...
        // Periodic reconnection timer for configured peers (30s)
        let mut reconnect_interval = tokio::time::interval(std::time::Duration::from_secs(30));
        reconnect_interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        // Skip the first immediate tick — peers are dialed on SET_PEER_CONFIG
        ...
        loop {
            tokio::select! {
                ...
                // Periodic reconnection to configured peers
                _ = reconnect_interval.tick() => {
                    self.attempt_reconnect_configured_peers().await;
                }
            }
        }
    ...
    /// Attempt to connect to all configured peers that we're not already connected to.
    /// Skips self-dial by detecting our own listen address.
    async fn attempt_reconnect_configured_peers(&self) {
        let peers = self.configured_peers.read().await.clone();
        if peers.is_empty() {
            return;
        }

        let libp2p_port = self.libp2p_port;
        for addr_str in peers {
            let handle = self.libp2p_handle.clone();
            tokio::spawn(async move {
                if let Some(libp2p_addr) = resolve_peer_to_multiaddr(&addr_str, libp2p_port).await {
            ...
            MessageType::SetPeerConfig => {
                ...
                if let Ok(json_str) = std::str::from_utf8(&msg.payload) {
                    ...
                    if let Ok(config) = serde_json::from_str::<serde_json::Value>(json_str) {
                        ...
                        info!(
                            "Parsed peer config: known={:?}, preferred={:?}",
                            known, preferred
                        );
                        ...
                        // Save for periodic reconnection
                        {
                            let mut configured = self.configured_peers.write().await;
                            *configured = all_peers.clone();
                        }

                        // Initial dial attempt with self-dial prevention
                        let libp2p_port = self.libp2p_port;
                        for addr_str in all_peers {
                            let handle = self.libp2p_handle.clone();
                            tokio::spawn(async move {
                                if let Some(libp2p_addr) =
                                    resolve_peer_to_multiaddr(&addr_str, libp2p_port).await
                                {
                                    handle.dial(libp2p_addr).await;
                                }
                            });
...
mod tests {
    ...
    // ---- Self-dial prevention tests ----

    #[test]
    fn test_is_local_address_loopback() {
        // 127.0.0.1 must always be detected as local
        assert!(is_local_address("127.0.0.1".parse().unwrap()));
    }

    #[test]
    fn test_is_local_address_unspecified() {
        // 0.0.0.0 must be detected as local
        assert!(is_local_address("0.0.0.0".parse().unwrap()));
    }

    #[test]
    fn test_is_local_address_v6_loopback() {
        assert!(is_local_address("::1".parse().unwrap()));
    }

    #[test]
    fn test_is_local_address_remote_ip() {
        // A remote public IP should NOT be detected as local
        assert!(!is_local_address("8.8.8.8".parse().unwrap()));
    }

    // ---- resolve_peer_to_multiaddr tests ----

    #[tokio::test]
    async fn test_resolve_ip_port_produces_multiaddr() {
        // Direct IP:port should yield /ip4/<ip>/udp/<port+1000>/quic-v1
        let result = resolve_peer_to_multiaddr("10.0.0.5:11625", 12625).await;
        assert!(result.is_some(), "Valid IP:port should produce a Multiaddr");
        let ma = result.unwrap().to_string();
        assert!(
            ma.contains("/ip4/10.0.0.5/udp/12625/quic-v1"),
            "Expected port+1000=12625, got: {}",
            ma
        );
    }

    #[tokio::test]
    async fn test_resolve_self_dial_loopback_blocked() {
        // 127.0.0.1:11625 with local_libp2p_port=12625 → self-dial → None
        let result = resolve_peer_to_multiaddr("127.0.0.1:11625", 12625).await;
        assert!(
            result.is_none(),
            "Self-dial to loopback should return None"
        );
    }

    #[tokio::test]
    async fn test_resolve_different_port_not_self_dial() {
        // 127.0.0.1:11626 with local_libp2p_port=12625 → port+1000=12626 ≠ 12625 → allowed
        let result = resolve_peer_to_multiaddr("127.0.0.1:11626", 12625).await;
        assert!(
            result.is_some(),
            "Different port on loopback should NOT be treated as self-dial"
        );
    }

    #[tokio::test]
    async fn test_resolve_bogus_hostname_returns_none() {
        // A hostname that cannot resolve should return None, not panic
        let result =
            resolve_peer_to_multiaddr("this-host-does-not-exist.invalid", 12625).await;
        assert!(
            result.is_none(),
            "Unresolvable hostname should return None"
        );
    }

    #[tokio::test]
    async fn test_resolve_localhost_hostname_returns_none() {
        // On most systems "localhost" resolves to ::1 (IPv6). Our multiaddr builder
        // only handles /ip4/, so this returns None. Self-dial for IPv4 is tested
        // by test_resolve_self_dial_loopback_blocked. In production K8s, pods have
        // IPv4 pod IPs detected by is_local_address().
        let result = resolve_peer_to_multiaddr("localhost", 12625).await;
        assert!(
            result.is_none(),
            "localhost should return None (IPv6 or self-dial)"
        );
    }
