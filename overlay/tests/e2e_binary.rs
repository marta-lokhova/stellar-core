...
mod ipc {
    ...
    pub fn send_message(stream: &mut UnixStream, msg_type: u32, payload: &[u8]) -> std::io::Result<()> {
...
mod tests {
    ...
    fn test_two_binaries_relay_scp() {
        ...
        let peer_config = r#"{"known_peers":["127.0.0.1:11710"],"preferred_peers":[],"listen_port":11711}"#;
        ...
        match result {
            Ok((msg_type, payload)) => {
                assert_eq!(msg_type, ipc::SCP_RECEIVED, "Should receive SCP_RECEIVED message");
