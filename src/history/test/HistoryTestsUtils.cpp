...
CatchupSimulation::catchupOnline(Application::pointer app, uint32_t initLedger,
                                 ...
                                 bool randomizeBufferedLedgers,
                                 std::vector<uint32_t> ledgersToInject)
{
    auto& lm = app->getLedgerManager();
    auto startCatchupMetrics = getCatchupMetrics(app);
    ...
    // this option will change the ordering of the triggerLedger and ledgers
    // after it
    if (randomizeBufferedLedgers)
    {
        for (uint32_t n = initLedger; n < triggerLedger; ++n)
        {
            externalize(n);
        }

        std::vector<uint32_t> bufferedLedgers(bufferLedgers);
        std::iota(bufferedLedgers.begin(), bufferedLedgers.end(),
                  triggerLedger + 1);
        std::random_shuffle(std::begin(bufferedLedgers),
                            std::end(bufferedLedgers));

        for (auto ledger : bufferedLedgers)
        {
            externalize(ledger);
        }

        // Move trigger ledger to end so we can test that any ledger greater
        // than checkpoint can trigger catchup
        externalize(triggerLedger);
    }
    else
    {
