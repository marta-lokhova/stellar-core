...
LedgerTxnRoot::Impl::dropOffers(bool rebuild)
{
    ...
    getSession() << "DROP TABLE IF EXISTS offers;";
    ...
    if (rebuild)
    {
        ...
        getSession()
            ...
            << "amount           BIGINT           NOT NULL CHECK (amount >= 0),"
               ...
               "PRIMARY KEY      (offerid)"
               ");";
        getSession() << "CREATE INDEX bestofferindex ON offers "
                        "(sellingasset,buyingasset,price,offerid);";
        getSession() << "CREATE INDEX offerbyseller ON offers "
                        "(sellerid);";
        if (!mApp.getDatabase().isSqlite())
        {
            getSession() << "ALTER TABLE offers "
