        ...
        SECTION("core")
        {
            ...
            SECTION("pull mode with 2 nodes")
            {
                ...
                auto advertCheck = [&](std::shared_ptr<Application> app) {
                    ...
                    auto demandTimeouts = om.mDemandTimeoutMeter.count();
                    auto hashesQueued =
                        app->getMetrics()
                            .NewMeter({"overlay", "flood", "advertized"},
                                      "message")
                            .count();
                    ...
                    // Each should send exactly one advert with exactly (nbTx /
                    // 2) hashes to each other, and demand & fulfill demands(s).
                    // The number of demands may depend on how many hashes are
                    // in one demand.

                    bool res = true;
                    res &= advertsSent == 1 && advertsRecvd == 1;
                    res &= hashesQueued == (nbTx / 2);
                    res &= demandFulfilled >= 1 && demandsSent >= 1;
                    res &= messagesUnfulfilled == 0;
                    res &= demandTimeouts == 0;
            ...
            SECTION("pull mode with 4 nodes")
            {
                ...
                simulation = Topologies::core(
                    numNodes, .666f, Simulation::OVER_TCP, networkID, cfgGen2);
                auto advertCheck = [&](std::shared_ptr<Application> app) {
                    if (!ackedTransactions(app))
                    {
                        return false;
                    ...
                    // every node has to advertize, demand, and fulfill demands
                    // at least once.
                    // No node should do so more than 3 * nbTx times.

                    bool res = true;

                    res &= 1 <= advertsSent && advertsSent <= 3 * nbTx;
                    res &= 1 <= advertsRecvd && advertsRecvd <= 3 * nbTx;

                    // If this node queued < 25 hashes and/or responded to < 25
                    // demands, then no other node can obtain the 25 txns that
                    // this node has.
                    res &= (nbTx / 4) <= hashesQueued;
                    res &= hashesQueued <= 3 * nbTx;
                    res &= (nbTx / 4) <= demandFulfilled;
                    res &= demandFulfilled <= 3 * nbTx;

                    res &= 1 <= demandsSent && demandsSent <= 3 * nbTx;

                    // Each advert must contain at least one hash.
                    res &= advertsSent <= hashesQueued;

                    // # demands fullfulled
                    // <= # demands sent
                    // <= # hashes sent
                    // <= # hashes queued.
                    res &= demandFulfilled <= hashesQueued;
