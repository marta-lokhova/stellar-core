...
TEST_CASE("flow control byte capacity", "[overlay][flowcontrol]")
{
    ...
    auto test = [&](bool shouldRequestMore) {
        ...
        SECTION("send more flow")
        {
            ...
            auto& sendMoreMeter = app1->getOverlayManager()
                                      .getOverlayMetrics()
                                      .mSendSendMoreMeter;
            auto& sendMoreRecvMeter = app2->getOverlayManager()
                                          .getOverlayMetrics()
                                          .mRecvSendMoreTimer;
...
TEST_CASE("connecting to saturated nodes", "[overlay][connections][acceptance]")
{
    ...
    UNSCOPED_INFO("wait for node to be disConnectedForTesting");
...
TEST_CASE("disConnectedForTesting topology recovery")
{
    ...
    auto doTest = [&](bool usePreferred) {
        ...
        // DisConnectedForTesting graph 0-1-2-3 and 4-5-6
